...
namespace ImGui
{
    ...
    class FileBrowser
    {
        ...
        public:
            ...
            void SetTypeFilters(const std::vector<std::string>& _typeFilters)
            {
                typeFilters_.clear();
        ...
        private:
            ...
            {
            //    fileRecords_ = { FileRecord{ true, "..", "[X] ..", "" } };
              fileRecords_ = { FileRecord{ true, "..", ICON_FA_ARROW_LEFT, "" } };
                ...
                for (auto& p : std::filesystem::directory_iterator(currentDirectory_))
                {
                    ...
                    try
                    {
                        ...
                        std::string folderstring = std::string(ICON_FA_FOLDER)+ "  ";
                        std::string filestring = std::string(ICON_FA_FILE) + "  ";

                        rcd.showName = (rcd.isDir ? folderstring : filestring) + u8StrToStr(p.path().filename().u8string());
            ...
            bool IsExtensionMatched(const std::filesystem::path& _extension) const
            {
                #ifdef _WIN32
